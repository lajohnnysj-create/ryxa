  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      ...
      if (!d.has_subscription) {
        // Fully gone from Stripe satisfies a cancel intent; never a reactivate.
        if (d.reason === 'stripe_not_found') return expectCancelAtPeriodEnd === true;
        continue; // no_local_sub_id -> cannot verify, keep trying
      }
      if (d.cancel_at_period_end === expectCancelAtPeriodEnd) return true;
