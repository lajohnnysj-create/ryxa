...
// Per-video cooldown for the self-heal Bunny lookup. The editor polls every
// ~5s while a video is processing, but Bunny's webhook usually arrives
// within a minute or two of encode completion. We don't need to re-ask Bunny
// every single poll. This module-level cache tracks the most recent time we
// asked about a given video, and short-circuits if we've asked recently.
// Lives only in this serverless function instance, which is fine: cold
// starts just trigger one extra lookup, and there's no correctness issue
// from stale cache (we'd just take one more poll cycle to self-heal).
var SELF_HEAL_COOLDOWN_MS = 30000;
var _lastSelfHealAt = {};
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (row.bunny_video_id && (dbStatus === 'processing' || dbStatus === 'uploading')) {
      // Skip the Bunny lookup if we asked about this video recently. The
      // webhook is the primary source of truth; the Bunny lookup is just a
      // fallback for when the webhook fails. Re-asking every 5s is wasteful.
      var lastAsk = _lastSelfHealAt[row.bunny_video_id] || 0;
      var elapsed = Date.now() - lastAsk;
      if (elapsed > SELF_HEAL_COOLDOWN_MS) {
        _lastSelfHealAt[row.bunny_video_id] = Date.now();
        ...
        if (live && live.bunny_video_status && live.bunny_video_status !== dbStatus) {
          ...
          // Clear the cache entry since this video is now resolved -
          // future polls won't trigger the in-flight branch anyway.
          delete _lastSelfHealAt[row.bunny_video_id];
        }
      }
    }
