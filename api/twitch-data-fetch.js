// Vercel serverless function - Twitch Data Fetch (creator-triggered)
// =====================================================================
// Triggered when a creator either:
//   1. Connects Twitch (fired once right after the OAuth callback succeeds)
//   2. Opens the Media Kit editor and cached data is missing or stale
//   3. Clicks the "Refresh from Twitch" button in the editor
//
// Authenticates the calling Ryxa user via their Supabase JWT, then calls the
// shared refreshTwitchData() helper (token refresh + profile + follower count).
//
// Endpoint: POST https://ryxa.io/api/twitch-data-fetch
...
module.exports = async function handler(req, res) {
  ...
  if (require('./lib/rate-limit').tooMany(req, res, 'tw-fetch', 10, 60000)) return;
  ...
  try {
    ...
    if (!result.ok) {
      if (result.error === 'Not connected to Twitch') {
    ...
    console.error('twitch-data-fetch failed:', e.message);
