// /api/tiktok-oauth-ticket.js
//
// Issues a short-lived signed "ticket" used to start the TikTok OAuth flow
// without putting a Supabase session token in the URL. Mirrors the YouTube /
// Instagram ticket pattern.
//
// Flow:
...
//   4. The dashboard navigates to /api/tiktok-oauth-start?ticket=<signed>.
//   5. The start endpoint verifies the signature and uses the embedded user_id.
//
// Signed with HMAC-SHA256 using TIKTOK_TICKET_SIGNING_SECRET. Valid 5 minutes.
...
module.exports = async function handler(req, res) {
  ...
  if (require('./lib/rate-limit').tooMany(req, res, 'tt-ticket', 30, 60000)) return;
