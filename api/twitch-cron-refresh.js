// Vercel serverless function - Twitch Cron Refresh (background)
// =====================================================================
// Called daily by Supabase pg_cron. Refreshes any twitch_connections rows
// where data_last_fetched_at is null OR older than the stale threshold, via the
// shared fetch helper (token refresh + profile + follower count).
//
// Authenticated via X-Cron-Secret header, must match CRON_SECRET env var.
// Server-to-server; no user JWT involved.
//
// Endpoint: POST https://www.ryxa.io/api/twitch-cron-refresh
...
// Must align with the pg_cron cadence in sql/twitch-cron-schedule.sql.
...
module.exports = async function handler(req, res) {
  ...
  for (const row of staleRows) {
    try {
      const result = await refreshTwitchData(row.user_id);
      if (result.ok) {
        refreshed++;
      } else {
        failed++;
