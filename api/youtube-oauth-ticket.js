// /api/youtube-oauth-ticket.js
//
// Issues a short-lived signed "ticket" used to start the YouTube OAuth flow
// without putting a Supabase session token in the URL. Mirrors the Google
// Calendar ticket pattern; YouTube reuses the same Google OAuth app but is a
// fully separate connection (separate table, scopes, signing secret).
//
// Flow:
...
//   4. The dashboard navigates to /api/youtube-oauth-start?ticket=<signed>.
//   5. The start endpoint verifies the signature and uses the embedded user_id.
//
// Signed with HMAC-SHA256 using YT_TICKET_SIGNING_SECRET. Valid for 5 minutes.
...
module.exports = async function handler(req, res) {
  ...
  if (require('./lib/rate-limit').tooMany(req, res, 'yt-ticket', 30, 60000)) return;
