...
function caRenderResults(r) {
  ...
  var html = ''
    ...
    + '<button data-ds-action="ca-reset" class="ds-s-35389e">Analyze another</button>'
    + '</div>';
...
function taRenderResults(r) {
  ...
  var html = ''
    ...
    // Try again
    + '<button data-ds-action="ta-reset" class="ds-s-5288e9">Analyze another thumbnail</button>';
...
function showCompatibleFilesModal() {
  ...
  var groups = [
    ...
    ['Video', 'MP4, MOV, WEBM (up to 100 MB per file)'],
...
// Load projects when AI Design Studio is opened
var _dsLoadedOnce = false;
...
function dsAltText() {
  ...
  if (typeof isPro === 'function' && !isPro()) {
    showDsMsg('error', 'AI Alt Text is a Pro feature. Upgrade to use it.');
    return;
  }

  var btn = document.getElementById('ds-alttext-btn');
  btn.disabled = true;
  btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" class="ds-s-f33c30" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg> Analyzing...';
  ...
  .then(function(data) {
    btn.disabled = false;
    btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M8 13h8"/><path d="M8 17h5"/></svg> AI Alt Text';
    if (data.error) {
      showDsMsg('error', data.error);
      return;
    }
    if (!data.alt) {
      showDsMsg('error', 'Could not generate alt text.');
      return;
    }
    // Show modal with the alt text and copy button
    var overlay = document.createElement('div');
    overlay.id = 'ds-alttext-modal';
    overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.7);backdrop-filter:blur(4px);z-index:9999;display:flex;align-items:center;justify-content:center;padding:24px;';
    overlay.innerHTML = '<div class="ds-s-141cd8">'
      + '<div class="bio-s-9998de">AI Generated Alt Text</div>'
      + '<div class="ds-s-731bc6" id="ds-alttext-result">' + escapeHtml(data.alt) + '</div>'
      + '<div class="course-s-b9bbe5">'
      + '<button data-ds-action="alt-text-copy" class="ds-s-a51526">Copy</button>'
      + '<button data-ds-action="close-alttext-modal" class="ds-s-dea7b5">Close</button>'
      + '</div>'
      + '</div>';
    overlay.onclick = function(e) { if (e.target === overlay) overlay.remove(); };
    document.body.appendChild(overlay);
  ...
  .catch(function(err) {
    ...
    btn.disabled = false;
    btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M8 13h8"/><path d="M8 17h5"/></svg> AI Alt Text';
    showDsMsg('error', 'Failed to generate alt text. Try again.');
  });
}

function dsAltTextCopy() {
  var text = document.getElementById('ds-alttext-result')?.textContent;
  if (!text) return;
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(text).then(function() {
      showDsMsg('success', 'Alt text copied to clipboard!');
      document.getElementById('ds-alttext-modal')?.remove();
    }).catch(function() {
      fallbackCopy(text);
      showDsMsg('success', 'Alt text copied to clipboard!');
      document.getElementById('ds-alttext-modal')?.remove();
...
function dsCaption() {
  ...
  if (typeof isPro === 'function' && !isPro()) {
    showDsMsg('error', 'AI Caption is a Pro feature. Upgrade to use it.');
    return;
  }

  var btn = document.getElementById('ds-caption-btn');
  btn.disabled = true;
  btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" class="ds-s-f33c30" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg> Generating...';
  ...
  .then(function(data) {
    ...
    if (!data.captions) { showDsMsg('error', 'Could not generate captions.'); return; }

    // Parse captions
    var sections = data.captions.split(/CAPTION \d+:\n?/i).filter(function(s) { return s.trim(); });
    var captionCards = sections.map(function(s, i) {
      var lines = s.trim();
      return '<div class="ds-s-9a127c">'
        + '<div class="ds-s-b4f017">' + ['Professional', 'Casual', 'Short & Punchy'][i] + '</div>'
        + '<div class="ds-s-c2aca4" id="ds-caption-' + i + '">' + escapeHtml(lines) + '</div>'
        + '<button data-ds-action="caption-copy" data-ds-idx="' + i + '" class="ds-s-a4361b">Copy</button>'
        + '</div>';
    }).join('');

    var overlay = document.createElement('div');
    overlay.id = 'ds-caption-modal';
    overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.7);backdrop-filter:blur(4px);z-index:9999;display:flex;align-items:center;justify-content:center;padding:24px;';
    overlay.innerHTML = '<div class="ds-s-5ca53c">'
      + '<div class="bio-s-9998de">AI Generated Captions</div>'
      + captionCards
      + '<button data-ds-action="close-caption-modal" class="ds-s-ee713d">Close</button>'
      + '</div>';
    overlay.onclick = function(e) { if (e.target === overlay) overlay.remove(); };
    document.body.appendChild(overlay);
  })
  .catch(function(err) {
    ...
    dsCaptionResetBtn();
    showDsMsg('error', 'Failed to generate captions. Try again.');
  });
}

function dsCaptionResetBtn() {
  var btn = document.getElementById('ds-caption-btn');
  if (btn) {
    btn.disabled = false;
    btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg> AI Caption';
  }
}

function dsCaptionCopy(idx) {
  var text = document.getElementById('ds-caption-' + idx)?.textContent;
  if (!text) return;
  if (navigator.clipboard && navigator.clipboard.writeText) {
  ...
  showDsMsg('success', 'Caption copied to clipboard!');
}

function dsRewriteText() {
  if (!dsCanvas) return;
  var obj = dsCanvas.getActiveObject();
  if (!obj || (obj.type !== 'i-text' && obj.type !== 'textbox')) {
    showDsMsg('error', 'Select a text object first.');
    return;
  }

  if (typeof isPro === 'function' && !isPro()) {
    showDsMsg('error', 'AI Rewrite is a Pro feature. Upgrade to use it.');
    return;
  }

  var originalText = obj.text || '';
  if (!originalText.trim()) {
    showDsMsg('error', 'Text is empty. Type something first.');
    return;
  }

  var btn = document.getElementById('ds-rewrite-btn');
  btn.disabled = true;
  btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" class="ds-s-f33c30" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg> Rewriting...';
  ...
  .then(function(data) {
    dsRewriteResetBtn();
    if (data.error) { showDsMsg('error', data.error); return; }
    if (!data.rewrites) { showDsMsg('error', 'Could not generate rewrites.'); return; }

    var sections = data.rewrites.split(/(?:PROFESSIONAL|CASUAL|CREATIVE):\n?/i).filter(function(s) { return s.trim(); });
    var labels = ['Professional', 'Casual', 'Creative'];
    var cards = sections.map(function(s, i) {
      var cleaned = s.trim();
      return '<div class="ds-s-9a127c">'
        + '<div class="ds-s-b4f017">' + (labels[i] || 'Option ' + (i+1)) + '</div>'
        + '<div class="bio-s-f0cb5a" id="ds-rewrite-' + i + '">' + escapeHtml(cleaned) + '</div>'
        + '<div class="ds-s-0ff9ff">'
        + '<button data-ds-action="apply-rewrite" data-ds-idx="' + i + '" class="bio-s-c43b40">Apply to canvas</button>'
        + '<button data-ds-action="rewrite-copy" data-ds-idx="' + i + '" class="ds-s-3e947b">Copy</button>'
        + '</div>'
        + '</div>';
    }).join('');

    var overlay = document.createElement('div');
    overlay.id = 'ds-rewrite-modal';
    overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.7);backdrop-filter:blur(4px);z-index:9999;display:flex;align-items:center;justify-content:center;padding:24px;';
    overlay.innerHTML = '<div class="ds-s-5ca53c">'
      + '<div class="bio-s-b3617b">AI Rewrite</div>'
      + '<div class="bio-s-c2bb16">Original: "' + escapeHtml(originalText.substring(0, 80)) + (originalText.length > 80 ? '...' : '') + '"</div>'
      + cards
      + '<button data-ds-action="close-rewrite-modal" class="ds-s-ee713d">Close</button>'
      + '</div>';
    overlay.onclick = function(e) { if (e.target === overlay) overlay.remove(); };
    document.body.appendChild(overlay);
  })
  .catch(function(err) {
    ...
    dsRewriteResetBtn();
    showDsMsg('error', 'Failed to generate rewrites. Try again.');
  });
}

function dsRewriteResetBtn() {
  var btn = document.getElementById('ds-rewrite-btn');
  if (btn) {
    btn.disabled = false;
    btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg> AI Rewrite';
  }
}

function dsApplyRewrite(idx) {
  var text = document.getElementById('ds-rewrite-' + idx)?.textContent;
  if (!text || !dsCanvas) return;
  var obj = dsCanvas.getActiveObject();
  if (obj && (obj.type === 'i-text' || obj.type === 'textbox')) {
    obj.set('text', text);
    dsCanvas.renderAll();
    showDsMsg('success', 'Text updated on canvas!');
    document.getElementById('ds-rewrite-modal')?.remove();
  } else {
    showDsMsg('error', 'Select the text object first, then apply.');
  }
}

function dsRewriteCopy(idx) {
  var text = document.getElementById('ds-rewrite-' + idx)?.textContent;
  if (!text) return;
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(text).catch(function() { fallbackCopy(text); });
  ...
  showDsMsg('success', 'Copied to clipboard!');
}
...
function dsAutoCrop() {
  ...
  if (typeof isPro === 'function' && !isPro()) {
    showDsMsg('error', 'AI Auto-Crop is a Pro feature. Upgrade to use it.');
    return;
  }

  var btn = document.getElementById('ds-autocrop-btn');
  btn.disabled = true;
  btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" class="ds-s-f33c30" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg> Analyzing...';
  ...
  setTimeout(function() {
    try {
      ...
      // Analyze at reduced size for speed
      var analyzeScale = Math.min(200 / Math.max(w, h), 1);
      var aW = Math.round(w * analyzeScale);
      var aH = Math.round(h * analyzeScale);
      ...
      // Build saliency map — find regions with high contrast/edges
      var salMap = new Float32Array(aW * aH);
      for (var y = 1; y < aH - 1; y++) {
        for (var x = 1; x < aW - 1; x++) {
          var idx = (y * aW + x) * 4;
          var lum = data[idx] * 0.299 + data[idx+1] * 0.587 + data[idx+2] * 0.114;
          // Compare with neighbors (gradient magnitude)
          var idxR = (y * aW + x + 1) * 4;
          var idxD = ((y + 1) * aW + x) * 4;
          var lumR = data[idxR] * 0.299 + data[idxR+1] * 0.587 + data[idxR+2] * 0.114;
          var lumD = data[idxD] * 0.299 + data[idxD+1] * 0.587 + data[idxD+2] * 0.114;
          var grad = Math.abs(lum - lumR) + Math.abs(lum - lumD);
          // Also boost saturation
          var maxC = Math.max(data[idx], data[idx+1], data[idx+2]);
          var minC = Math.min(data[idx], data[idx+1], data[idx+2]);
          var sat = maxC > 0 ? (maxC - minC) / maxC : 0;
          salMap[y * aW + x] = grad + sat * 30;
        }
      }

      // Find bounding box of salient region (top 30% of saliency)
      var vals = Array.from(salMap).sort(function(a, b) { return b - a; });
      var threshold = vals[Math.floor(vals.length * 0.3)] || 0;

      var minX = aW, minY = aH, maxX = 0, maxY = 0;
      for (var y2 = 0; y2 < aH; y2++) {
        for (var x2 = 0; x2 < aW; x2++) {
          if (salMap[y2 * aW + x2] >= threshold) {
            if (x2 < minX) minX = x2;
            if (x2 > maxX) maxX = x2;
            if (y2 < minY) minY = y2;
            if (y2 > maxY) maxY = y2;
      ...
      var bw = maxX - minX;
      var bh = maxY - minY;
      var padX = Math.round(bw * 0.1);
      var padY = Math.round(bh * 0.1);
      ...
      var cropX = Math.round(minX / analyzeScale);
      var cropY = Math.round(minY / analyzeScale);
      var cropW = Math.round((maxX - minX) / analyzeScale);
      var cropH = Math.round((maxY - minY) / analyzeScale);
      ...
      fabric.Image.fromURL(dataUrl, function(newImg) {
        var fitScale = Math.min(
          ...
          (dsCanvas.height * 0.8) / cropH,
          1
        );
        newImg.set({
          left: (dsCanvas.width - cropW * fitScale) / 2,
          top: (dsCanvas.height - cropH * fitScale) / 2,
          scaleX: fitScale,
          scaleY: fitScale
        });
        dsCanvas.remove(obj);
        ...
        dsCanvas.renderAll();
        btn.disabled = false;
        btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2v4M18 2v4M6 18v4M18 18v4M2 6h4M2 18h4M18 6h4M18 18h4"/><rect x="6" y="6" width="12" height="12" rx="1"/></svg> AI Auto-Crop';
        showDsMsg('success', 'Auto-cropped to ' + cropW + ' × ' + cropH + 'px');
      });
    } catch(err) {
      console.error('Auto-crop error:', err);
      btn.disabled = false;
      btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2v4M18 2v4M6 18v4M18 18v4M2 6h4M2 18h4M18 6h4M18 18h4"/><rect x="6" y="6" width="12" height="12" rx="1"/></svg> AI Auto-Crop';
      showDsMsg('error', 'Auto-crop failed. Try a different image.');
    }
  }, 50);
}
...
dsRegisterAction('close-alttext-modal', () => {
  var m = document.getElementById('ds-alttext-modal');
  if (m) m.remove();
});
dsRegisterAction('close-caption-modal', () => {
  var m = document.getElementById('ds-caption-modal');
  if (m) m.remove();
});
dsRegisterAction('close-rewrite-modal', () => {
  var m = document.getElementById('ds-rewrite-modal');
  if (m) m.remove();
});

// AI tool actions in modals
dsRegisterAction('alt-text-copy', () => dsAltTextCopy());
dsRegisterAction('caption-copy', (e, el) => dsCaptionCopy(parseInt(el.dataset.dsIdx, 10)));
dsRegisterAction('apply-rewrite', (e, el) => dsApplyRewrite(parseInt(el.dataset.dsIdx, 10)));
dsRegisterAction('rewrite-copy', (e, el) => dsRewriteCopy(parseInt(el.dataset.dsIdx, 10)));

// Analyzer reset (Contract Analyzer / Thumbnail Analyzer are inside DS)
