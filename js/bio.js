...
const BIO_THEMES = [
  ...
  { key:'malachite',  name:'Malachite',image:'/bgtemplates/6.webp', colors:{bg:'#0A2318',card:'#0B2A1E',text:'#F3EEE0',accent:'#D4AF37'}, pro:false },
  // ----- Color themes (free) -----
...
function deriveThemeFromCustom(colors) {
  ...
  const surface = c.cardTransparent ? 'transparent' : c.card;
  return {
    bg: c.bg,
    bg2: surface,
    surface: surface,
...
function toggleCardTransparent(checked) {
  if (!bioState.custom_theme) bioState.custom_theme = { bgUrl: '', bgOpacity: 0.4, colors: { ...CUSTOM_THEME_DEFAULTS }, applied: true };
  if (!bioState.custom_theme.colors) bioState.custom_theme.colors = { ...CUSTOM_THEME_DEFAULTS };
  bioState.custom_theme.colors.cardTransparent = !!checked;
  syncBioCustomEditorUI();
  schedulePreviewUpdate();
}
...
function bioRowTypeMeta(link) {
  ...
  if (link.isVideoBlock) {
    return {
      key: 'video',
      label: 'Videos',
      icon: '<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="1" y="5" width="22" height="14" rx="4" fill="#FF0000"/><path d="M9.8 8.5 16 12l-6.2 3.5z" fill="#fff"/></svg>'
    };
  }
  if (link.isTikTokBlock) {
    return {
      key: 'tiktok',
      label: 'TikTok',
      icon: '<svg viewBox="0 0 24 24" aria-hidden="true"><path transform="translate(-0.9 0)" fill="#25F4EE" d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z"/><path transform="translate(0.9 0)" fill="#FE2C55" d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z"/><path fill="#fff" d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z"/></svg>'
    };
  }
...
function renderLinkCollapsed(link, dragSvg, editSvg) {
  ...
  } else if (link.isImageBlock) {
    if (link.photoUrl) {
      thumb = `<img alt="Image thumbnail" src="${escapeHtml(link.photoUrl)}" class="bio-s-3323bf">`;
    }
    title = 'Image';
...
// Upload handler for a Single Image block. Compresses to WebP (max 1200px box,
// ~160KB), then reads the output dimensions so the renderers can reserve the
// aspect ratio (no layout shift). Available on all plans.
async function onImagePhotoSelected(input, linkId) {
  const file = input.files[0];
  input.value = '';
  if (!file) return;
  if (!file.type.startsWith('image/')) { showBioStatus('error', 'Please upload an image.'); return; }
  ...
  try {
    showBioStatus('info', 'Uploading…');
    const blob = await compressBgImage(file, 1200, 1200, 160 * 1024);
    // Read final dimensions from the compressed blob for the aspect ratio.
    ...
    const fileName = `image-${linkId}-${Date.now()}-${Math.random().toString(36).slice(2,8)}.webp`;
    const path = `${currentUser.id}/${fileName}`;
    const { error: upErr } = await sb.storage.from('bio-photos').upload(path, blob, { contentType: 'image/webp', upsert: false });
    if (upErr) throw upErr;
    const { data: { publicUrl } } = sb.storage.from('bio-photos').getPublicUrl(path);
    ...
    renderBioLinks();
    schedulePreviewUpdate();
    showBioStatus('success', 'Image uploaded. Remember to save.');
  } catch (e) {
    console.error(e);
    showBioStatus('error', `Upload failed: ${e.message || 'unknown'}`);
  }
...
function renderLinkExpanded(link, dragSvg) {
  ...
  if (link.isImageBlock) {
    return `<div class="bio-link-row" data-id="${link._id}">
      <div class="bio-link-header">
        <div class="bio-link-drag" aria-label="Drag to reorder">${dragSvg}</div>
        <span class="bio-featured-badge bio-s-04da54" >Image</span>
        <div class="bio-s-7623f0"></div>
        <button class="bio-link-remove" data-bio-action="remove-link" data-bio-id="${link._id}">Remove</button>
      </div>
      <div class="bio-s-e289c0">Upload an image, horizontal or vertical. It's compressed automatically and shown at full width on your page. Tap the box to replace it.</div>
      <div class="bio-image-slot">
        ${link.photoUrl ? `<img alt="Image preview" src="${escapeHtml(link.photoUrl)}">` : '<span>+ Upload image</span>'}
        <input type="file" accept="image/*" aria-label="Upload image" data-bio-action="image-photo-selected" data-bio-event="change" data-bio-id="${link._id}">
      </div>
      <button type="button" data-bio-action="save-link-row" data-bio-id="${link._id}"
        class="bio-s-c7cf47">
        Save
  ...
  if (link.isTwitchBlock) {
    ...
    return `<div class="bio-link-row" data-id="${link._id}">
      <div class="bio-link-header">
        <div class="bio-link-drag" aria-label="Drag to reorder">${dragSvg}</div>
        <span class="bio-featured-badge bio-s-04da54" >Twitch</span>
        <div class="bio-s-7623f0"></div>
        <button class="bio-link-remove" data-bio-action="remove-link" data-bio-id="${link._id}">Remove</button>
      </div>
...
// and videos are checked before the bare-channel pattern; reserved path
// segments are excluded so e.g. twitch.tv/videos/... isn't read as a channel.
...
// Twitch embed iframe src. parent must match the hosting domain (bios live on
// www.ryxa.io). autoplay off so a live channel never auto-blasts.
...
function buildPreviewHTML() {
  ...
  *{-webkit-user-select:none!important;user-select:none!important;-webkit-touch-callout:none!important;}
  ...
  .lk-thumb-body{flex:1;min-width:0;display:flex;flex-direction:column;justify-content:center;align-items:center;text-align:center;padding:8px 18px;gap:2px;overflow:hidden;}
  .lk.lk-thumb:not(.link-half) .lk-thumb-body .lk-t{display:-webkit-box;-webkit-line-clamp:1;-webkit-box-orient:vertical;overflow:hidden;}
  .lk.lk-thumb:not(.link-half) .lk-thumb-body .lk-d{display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;}
...
function buildPreviewLink(l, t) {
  ...
  if (l.isImageBlock) {
    if (!l.photoUrl) {
      return `<div style="width:100%;aspect-ratio:16/10;border-radius:14px;background:${t.surface};border:1px dashed ${t.border};display:flex;align-items:center;justify-content:center;color:${t.muted};font-size:12px;">Image</div>`;
    }
    const dim = (l.imgW && l.imgH) ? ` width="${l.imgW}" height="${l.imgH}"` : '';
    return `<div style="width:100%;border-radius:14px;overflow:hidden;"><img src="${escapeHtml(l.photoUrl)}"${dim} alt="" style="width:100%;height:auto;display:block;"></div>`;
  }
...
// Tile metadata for the rate card grid: each predefined rate maps to a platform
// icon plus a short offering label (the icon carries the platform, so the label
// just names the format). Custom rates fall back to the 'other' icon below.
const _mkIgSvg = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2.2c3.2 0 3.6 0 4.85.07 1.17.05 1.8.25 2.22.41.56.22.96.48 1.38.9.42.42.68.82.9 1.38.16.42.36 1.05.41 2.22.06 1.26.07 1.64.07 4.82s-.01 3.57-.07 4.82c-.05 1.17-.25 1.8-.41 2.22a3.72 3.72 0 0 1-.9 1.38c-.42.42-.82.68-1.38.9-.42.16-1.05.36-2.22.41-1.26.06-1.64.07-4.82.07s-3.57-.01-4.82-.07c-1.17-.05-1.8-.25-2.22-.41a3.72 3.72 0 0 1-1.38-.9 3.72 3.72 0 0 1-.9-1.38c-.16-.42-.36-1.05-.41-2.22C2.21 15.57 2.2 15.19 2.2 12s.01-3.57.07-4.82c.05-1.17.25-1.8.41-2.22.22-.56.48-.96.9-1.38.42-.42.82-.68 1.38-.9.42-.16 1.05-.36 2.22-.41C8.43 2.21 8.81 2.2 12 2.2M12 0C8.74 0 8.33.01 7.05.07 5.78.13 4.9.33 4.14.63a5.92 5.92 0 0 0-2.13 1.39A5.92 5.92 0 0 0 .62 4.14C.33 4.9.13 5.78.07 7.05.01 8.33 0 8.74 0 12c0 3.26.01 3.67.07 4.95.06 1.27.26 2.15.56 2.91a5.92 5.92 0 0 0 1.39 2.13c.66.66 1.32 1.06 2.13 1.39.76.3 1.64.5 2.91.56C8.33 23.99 8.74 24 12 24c3.26 0 3.67-.01 4.95-.07 1.27-.06 2.15-.26 2.91-.56a5.92 5.92 0 0 0 2.13-1.39c.66-.66 1.06-1.32 1.39-2.13.3-.76.5-1.64.56-2.91.06-1.28.07-1.69.07-4.95s-.01-3.67-.07-4.95c-.06-1.27-.26-2.15-.56-2.91a5.92 5.92 0 0 0-1.39-2.13A5.92 5.92 0 0 0 19.86.62c-.76-.3-1.64-.5-2.91-.56C15.67.01 15.26 0 12 0Zm0 5.84a6.16 6.16 0 1 0 0 12.32 6.16 6.16 0 0 0 0-12.32Zm0 10.16a4 4 0 1 1 0-8 4 4 0 0 1 0 8Zm6.41-11.88a1.44 1.44 0 1 0 0 2.88 1.44 1.44 0 0 0 0-2.88Z"/></svg>';
const _mkTtSvg = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5.8 20.1a6.34 6.34 0 0 0 10.86-4.43V8.83a8.16 8.16 0 0 0 4.77 1.52V6.9a4.85 4.85 0 0 1-1.84-.21Z"/></svg>';
const _mkYtSvg = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M23.5 6.2a3 3 0 0 0-2.1-2.12C19.54 3.58 12 3.58 12 3.58s-7.54 0-9.4.5A3 3 0 0 0 .5 6.2C0 8.07 0 12 0 12s0 3.93.5 5.8a3 3 0 0 0 2.1 2.12c1.86.5 9.4.5 9.4.5s7.54 0 9.4-.5a3 3 0 0 0 2.1-2.12C24 15.93 24 12 24 12s0-3.93-.5-5.8ZM9.55 15.57V8.43L15.82 12l-6.27 3.57Z"/></svg>';
// 'Other' icon for custom rates: a simple tag/label glyph.
const _mkOtherSvg = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg>';

const MK_RATE_META = {
  ig_reel:  { icon: _mkIgSvg, short: 'Reel' },
  ig_post:  { icon: _mkIgSvg, short: 'Post' },
  ig_story: { icon: _mkIgSvg, short: 'Story' },
  tiktok:   { icon: _mkTtSvg, short: 'Video' },
  youtube:  { icon: _mkYtSvg, short: 'Video' },
};
...
// Fill the sheet's Share pill with the saved public URL. If no username has
// been saved yet there is no live link; show that and leave the URL empty so
// the copy action no-ops (mirrors the dashboard card's 'ryxa.io/...' guard).
...
// Same feedback as the dashboard welcome link card: the URL text flips to a
// green 'Copied!' for a moment, then restores.
