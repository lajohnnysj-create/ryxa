...
async function mountProductDescEditor() {
  ...
  // Same toolbar as courses/booking: emphasis, lists, headings (H2/H3 only),
  // links, and images. The image button routes through the same
  // compressLessonImage + course-images bucket pipeline used by the
  // course-description, lesson, and booking-description editors. Default
  // image size is "small"; creator clicks the image to use the S/M/L toolbar.
  // Bucket is shared across all description-image features for simplicity.
      ...
      toolbar: {
        ...
        handlers: {
          image: function() {
            ...
            if (imgCount >= MAX_PRODUCT_DESC_IMAGES) {
              showModalAlert('Image limit reached', 'You can add up to ' + MAX_PRODUCT_DESC_IMAGES + ' images per product description. Remove one before adding another.');
              return;
            }
            ...
            input.onchange = async function() {
              ...
              try {
                ...
                // The server-side DOMPurify hook backstops the alt on read, but
                // we set it here so the editor DOM matches the persisted shape
                // immediately.
                setTimeout(function() {
                ...
                console.error('Product description image upload error:', e);
  ...
  if (cleanInit && /<img/i.test(cleanInit)) {
    try {
      ...
      console.warn('Failed to re-apply product description image size classes after seed:', e);
    }
  }
  ...
  // Quill -> textarea sync on every edit. Enforces the char limit and
  // image-count limit by reverting via history.undo when exceeded. Image-cap
  // is gated on source === 'user' so the initial seed paste (programmatic)
  // doesn't trip the cap and undo its own work.
  quill.on('text-change', function(delta, oldDelta, source) {
    ...
    if (source === 'user') {
      ...
      if (imgCount > MAX_PRODUCT_DESC_IMAGES) {
        ...
        _productDescSyncInProgress = false;
        showModalAlert('Image limit reached', 'You can add up to ' + MAX_PRODUCT_DESC_IMAGES + ' images per product description.');
        return;
      }
    }
  ...
  // Wire up S/M/L image sizing. Same toolbar widget as the course-desc,
  // lesson, and booking editors. Save callback runs when the creator clicks
  // S/M/L (Quill's text-change doesn't fire for class-only attribute
  // changes), syncing the resized HTML through the sanitizer to the textarea.
