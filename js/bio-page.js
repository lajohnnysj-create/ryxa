...
function buildLink(link) {
  ...
  // Spotify embed — single track/album/playlist/artist/episode/show. Tracks and
  // episodes get the compact player; the rest get the taller tracklist player.
  ...
  if (link.isTipBlock) {
    ...
    return '<div class="tip-card' + halfClass + '" id="tip-card">'
      ...
      + '<div class="tip-card-amts">' + chips + '<input type="text" id="tip-custom" class="tip-custom" inputmode="decimal" maxlength="6" placeholder="$0.00" aria-label="Custom tip amount"></div>'
...
// Share icon: created and appended into #wrap after render (both the SSR
// fast path and the client fallback call this), so the SSR wrap rewrite
// never interacts with it. Copies the page link; confirms with a checkmark.
function mountBioShareButton() {
  var wrap = document.getElementById('wrap');
  if (!wrap || document.getElementById('bio-share-btn')) return;
  var btn = document.createElement('button');
  btn.id = 'bio-share-btn';
  btn.type = 'button';
  btn.setAttribute('aria-label', 'Copy page link');
  btn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>';
